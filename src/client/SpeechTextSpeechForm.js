...
class SpeechTextSpeech extends React.Component {
  ...
  componentDidMount() {
    ...
    socket.on('audiodata', (data) => {
      ...
      this.stopListening();
      playAudioBuffer(audioFromString, this.audioContext, true);

    });
  }
  componentWillUnmount() {
    console.log("sts unmounted");
    this.stopListening();
  ...
  async startListening(){
    if(!this.state.audio){
      startStreaming(this.audioContext, true);
  ...
  stopListening(){
    if(this.state.audio){
      ...
      stopStreaming(this.audioContext, true);
