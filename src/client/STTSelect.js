...
class STTSelect extends React.Component {
  constructor(props) {
    super(props);
  }
  ...
  handleSTTChange = (event) => {
    this.setState({ sttLanguage: event.target.value });
    console.log("stt language selected is " + event.target.value);
    let socket = this.props.socket;
    socket.emit("sttLanguageCode", event.target.value);
  };
  ...
  render() {
    ...
    return (
        <FormControl className={classes.formControl}>
          ...
          }} htmlFor="stt-language">Translate From</InputLabel>
          <Select
            native
...
export default withStyles(styles)(STTSelect);
